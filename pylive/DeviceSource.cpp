...
// V4L2 FramedSource
// ---------------------------------
V4L2DeviceSource* V4L2DeviceSource::createNew(UsageEnvironment& env,
		DeviceInterface * device, unsigned int queueSize) {
	V4L2DeviceSource* source = NULL;
	if (device) {
		source = new V4L2DeviceSource(env, device, queueSize);
	}
	return source;
}
...
V4L2DeviceSource::V4L2DeviceSource(UsageEnvironment& env,
		DeviceInterface * device, unsigned int queueSize) :
		FramedSource(env), m_in("in"), m_out("out"), m_device(device), m_queueSize(
				queueSize) {
	...
	buffer = new char[m_device->getBufferSize()];
}
...
void V4L2DeviceSource::processFrame(char * frame, int frameSize,
		const timeval &ref) {
	timeval tv;
	gettimeofday(&tv, NULL);
	timeval diff;
...
void V4L2DeviceSource::queueFrame(char * frame, int frameSize,
		const timeval &tv) {
	pthread_mutex_lock(&m_mutex);
...
std::list<std::pair<unsigned char*, size_t> > V4L2DeviceSource::splitFrames(
		unsigned char* frame, unsigned frameSize) {
	std::list < std::pair<unsigned char*, size_t> > frameList;
	if (frame != NULL) {
		frameList.push_back(
				std::pair<unsigned char*, size_t>(frame, frameSize));
	}
	return frameList;
}
